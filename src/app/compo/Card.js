...
export default function Card({post}) {
  ...
  <div className="bg-grey rounded-lg shadow-md w-full  h-full">
    ...
    <div className="flex items-center px-4 py-3 border-b">
      <img src={!post.author.avatar && "https://api.dicebear.com/8.x/adventurer/svg?seed="+post.author.username || post.author.avatar} alt="Profile Picture" className="h-10 w-10 rounded-full mr-2 bg-contain	" />
      <div>
        <Link href={"/"+post.author.username} ><h3 className="text-lg font-semibold">{post.author.name} <span className="text-sm text-gray-600">@{post.author.username}</span></h3> </Link>
