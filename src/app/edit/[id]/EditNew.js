...
function EditNew({post}) {
  const [content, setContent] = useState(post.content);
  const [title, setTitle] = useState(post.title);
  const [authorId, setAuthorId] = useState('');
  const [image, setImage] = useState(post.contentURL);
  let box = useRef('box');
  const [imgsrc, setImgsrc] = useState(post.contentURL);
  const [posting, setPosting] = useState(0);
  const [postmode, setPostmode] = useState(post.type);
        ...
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-1">
            ...
            <label htmlFor="title" className=' -mt-4'>
             ...
             <input 
              id="title"
              placeholder={(postmode === "article") && "Title of your Article" || postmode === "image" && "Caption your Image 🔥" || postmode === "video" && "Video Title ✨" }
              className='w-full bg-transparent p-2 mb-2 border-black dark:border-white border-b'
              type="text"
              name='title'
